...
class Levenshtein {
    ...
    static similarity(a, b) {
        if (a.length == 0)
            return b.length; // If a is empty, distance is length of b (all insertions)
        if (b.length == 0)
            return a.length; // If b is empty, distance is length of a (all deletions)
        var matrix = [];
        ...
        for (let i = 1; i <= b.length; i++) {
            for (let j = 1; j <= a.length; j++) {
                if (b.charAt(i - 1) == a.charAt(j - 1)) {
                    ...
                    matrix[i][j] = Math.min(matrix[i - 1][j - 1] + 1, // Substitution cost (previous + 1)
                    Math.min(matrix[i][j - 1] + 1, // Insertion cost (previous row + 1)
